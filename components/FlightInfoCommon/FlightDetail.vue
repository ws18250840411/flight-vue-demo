<template>
  ...
  <div class="row flight-detail" :class="{'p-b-0': leg===1}">
    <div class="col-xs-4 text-right flight-depart">
      <div class="flight-time" :class="{transit: leg===2}" v-if="!transitOverMidNight">{{ departTime }}</div>
      <div class="flight-time-with-overmidnight" v-else>
        <div class="flight-time" :class="{transit: leg===2}">{{ departTime }}</div>
    ...
    <div class="col-xs-4 text-left flight-arrive">
      <div class="flight-time"  :class="{transit: leg===1 && transitExpanded}" v-if="!overMidNight">{{ arriveTime }}</div>
      <div class="flight-time-with-overmidnight" v-else>
        <div class="flight-time" :class="{transit: leg===1 && transitExpanded}">{{ arriveTime }}</div>
        <div class="flight-time-overmidnight hover-container">
          <small class="overmidnight-text abbr"></small>
          <div class="hover-box">
      ...
      <div class="flight-airport" v-if="leg===1 && !transitExpanded">{{ getAirportTerminal(transitFlight.arriveInfo) }}</div>
      <div class="flight-airport" v-else>{{ getAirportTerminal(flight.arriveInfo) }}</div>
    </div>
  </div>
</template>

<script>
  import {getDuration} from '../../js/flightInfoFns'
  import format from 'date-fns/format'
  import isSameDay from 'date-fns/is_same_day'

  export default {
    props: ['flight', 'leg', 'firstDepartDateTime', 'transitExpanded', 'transitTime', 'transitFlight'], // 分别为要展示的航班、第几航段、首航段起飞时间、中转航班是否已展开、中转时长、中转航班完整信息
    methods: {
      getAirportTerminal (flightLegEndInfo) {
        //避免没有机场名称跟航站楼
        if(!(flightLegEndInfo.airport)){
          flightLegEndInfo.airport = '';
        }
        if(!(flightLegEndInfo.terminal)){
          flightLegEndInfo.terminal = '';
        }
